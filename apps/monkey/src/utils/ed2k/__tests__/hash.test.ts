...
describe('ed2k hash', () => {
  ...
  /**
   * ============================================================================
   * 步骤2：验证 CSP 禁止 WebAssembly
   * ============================================================================
   * 目标：115 页面禁止 unsafe-eval 时仍使用纯 TypeScript 生成标准 MD4。
   * 数据源：会抛出 CSP CompileError 的 WebAssembly.compile 桩。
   * 操作：
   * 1) 禁止 WebAssembly 编译
   * 2) 核对 RFC 1320 摘要且未访问编译器
   */
  it('does not compile WebAssembly when CSP blocks unsafe-eval', async () => {
    logger.info('开始验证 ED2K CSP 兼容路径')

    // 2.1 模拟截图中的 115 CSP 拒绝 WebAssembly.compile
    const compile = vi.spyOn(WebAssembly, 'compile').mockImplementation(() => {
      throw new WebAssembly.CompileError('unsafe-eval is not allowed')
    })

    // 2.2 哈希结果保持标准，且实现不触碰 WebAssembly
    await expect(hashEd2kPart(new TextEncoder().encode('message digest')))
      .resolves
      .toBe('D9130A8164549FE818874806E1C7014B')
    expect(compile).not.toHaveBeenCalled()
    compile.mockRestore()

    logger.info('ED2K CSP 兼容路径验证完成')
  })
  ...
  it('adds the protocol empty-part hash at an exact part boundary', async () => {
    ...
    // 3.1 全零标准分块摘要由独立 OpenSSL MD4 参考实现生成
