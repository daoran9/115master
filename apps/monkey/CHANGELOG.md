...
- 低优先级番号缓存先显示现有结果，再按十分钟冷却后台探测 JavLibrary；后台升级与页面级三路联网并发共用槽位，避免过期缓存同时放大请求。
- 新版文件接口索引限制为最多 2048 个文件项，淘汰最早写入项，避免连续浏览目录时内存无界增长。
- ED2K 网络层改为双通道，每个请求读取四个标准块；请求失败时刷新对应临时下载地址并最多重试三次，保留已完成摘要后从失败批次续算，同时限制进度刷新频率。
- 旧版和新版文件列表支持为文件名主体是番号的 ISO 光盘镜像加载影片详情，不误识软件镜像。
- 旧版 115 页面恢复历史插件的浅色番号详情卡、150 像素视频预览、列表留白和播放入口文字；竖版或异常比例封面完整显示；新版页面与 MASTER 保持现有布局。
- 旧版 115 页面补齐原版详情来源、加载状态和操作入口顺序，视频预览从五帧增加到八帧。
- 番号详情优先加载 DMM 实体 DVD 横封套；实体图缺失或为占位图时继续选择其他来源的真实横封套，全部没有横图才按番号、现有详情标题查询 FANZA 官方数字封面，不覆盖原有标题、演员等资料字段。
- 播放器剧照优先加载 FANZA/DMM 官方高清图，其他资料源按原图、缩略图回退；点击放大时复用已加载的 Blob，不再无 Referer 直连外站。
- 旧版页面恢复历史云下载按钮颜色、女优头像浅色边框和返回目录按钮样式。
- 新版页面 Fusion 工具组紧跟预览按钮；选择文件时保留原位，由顶部批量操作栏按原生层级覆盖。
- 旧版页面保留原版 115Master 入口；Fusion 工具组仅应用于新版页面，避免旧版搜索页出现重复启动入口。
- JavDB 延迟加载封面优先读取真实 `data-src`，测试桥同步开放 `jdbstatic.com` 图片域，避免详情正常但封面误读占位图或无法现场探测。
